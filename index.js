...
function loadPlants(id){
    ...
    .then ((data)=>{
        console.log(data)
        cardContainer.innerHTML+=`    <div class="p-4 bg-white"><img src="${data.plants.image}" alt=""><h1 class="font-bold" id="name-${data.plants.id}">${data.plants.name}</h1><p class="text-gray-700">${data.plants.description}</p><div class="flex justify-between my-4"><button class="bg-[#DCFCE7] text-[#00ffa6] w-26 rounded-xl h-8">Fruit Tree</button><p class="font-semibold">৳<span>${data.plants.price}</span></p></div> <div class="flex justify-center"><button class="text-white  bg-green-600 h-12 w-full rounded-4xl">
          Add to Cart
        </button></div></div>`
...
modal.innerHTML+=`<!-- Open the modal using ID.showModal() method -->
